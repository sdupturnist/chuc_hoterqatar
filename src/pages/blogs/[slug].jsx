...
export default function BlogSingle({ singleBLogsData_, blogData_ }) {
  ...
  return (
    singleBLogsData_ && (
      <>
        <Metatags seo={singleBLogsData_?.data?.posts?.data[0]?.attributes?.seo} />
        <Layout page="blog-single">
            ...
            <div className="mx-auto 2xl:w-[60%] xl:w-[70%] grid sm:gap-[50px] gap-[30px] xl:py-[70px] lg:py-[50px] py-[30px]">
              {isLoading ? (
                ...
                <div className="grid sm:gap-[30px] gap-[20px]">
                  <h1 className="sm:text-[36px] text-[22px] font-semibold">{singleBLogsData?.Title}</h1>
                  <Images
                    width={banner?.width}
                    height={banner?.height}
                    quality={100}
                    placeholder={true}
                    imageurl={adminUrl + banner?.url}
                    classes={'mx-auto w-full block rounded-[8px] object-cover sm:min-h-[450px]'}
                    alt={banner?.alternativeText}
                    title={banner?.alternativeText}
                  />
                  <div className="grid gap-[15px]">
                    <div className="content grid gap-[16px]">
                      <div dangerouslySetInnerHTML={{ __html: singleBLogsData?.Content[0]?.children[0]?.text || '' }} />
                    </div>
                    <p className="mt-[16px]">
                      {singleBLogsData?.Author} <span className="text-[10px] text-gray-300 px-[6px]">|</span> {formatBlogDate(singleBLogsData?.PostDate)}
                    </p>
                  </div>
                </div>
              )}
              ...
              {blogs.length !== 0 && (
                <div className="sm:py-[50px] py-[30px] justify-between gap-[40px] border-t border-gray-200 border-solid">
...
export async function getStaticPaths() {
  try {
    ...
    const slugs = data.data.posts.data.map(post => ({ params: { slug: post.attributes.Slug } }));

    return {
      paths: slugs,
      fallback: 'blocking', // 'blocking' will serve a static 404 page until the new static page is built
    };
  } catch (error) {
